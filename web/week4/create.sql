...
(
    ...
    session_number SMALLINT NOT NULL # 1-6, Sat. Morning - Women's 
);
...
CREATE TABLE public.user(id SERIAL NOT NULL PRIMARY KEY,username VARCHAR(100) NOT NULL UNIQUE,password VARCHAR(100) NOT NULL,display_name VARCHAR(100) NOT NULL);
CREATE TABLE public.speaker(id SERIAL NOT NULL PRIMARY KEY,name VARCHAR(100) NOT NULL);
CREATE TABLE public.conference(id SERIAL NOT NULL PRIMARY KEY,year SMALLINT NOT NULL,is_spring BOOLEAN NOT NULL,session_number SMALLINT NOT NULL);
CREATE TABLE public.talk(id SERIAL NOT NULL PRIMARY KEY,speaker_id INT NOT NULL REFERENCES public.speaker(id),conference_id INT NOT NULL REFERENCES public.conference(id),name VARCHAR(100) NOT NULL);
CREATE TABLE public.note(id SERIAL NOT NULL PRIMARY KEY,user_id INT NOT NULL REFERENCES public.user(id),talk_id INT NOT NULL REFERENCES public.talk(id),note_text TEXT NOT NULL);
